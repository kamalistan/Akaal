...
export function useDialerCallSubscription(userEmail) {
  ...
  const fetchCurrentCall = useCallback(async () => {
    if (!userEmail || isLoadingRef.current) return;
  ...
  useEffect(() => {
    ...
    const channelName = `dialer_call_${userEmail}_${Date.now()}`;
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.log('Dialer call update received:', payload.eventType, payload.new?.status);

          if (payload.eventType === 'DELETE') {
            if (payload.old.lead_id === null) {
              setCurrentCall(null);
            }
          } else if (payload.new && payload.new.lead_id === null) {
      ...
      .subscribe((status) => {
        console.log('Dialer subscription status:', status);
  ...
  }, [userEmail, fetchCurrentCall, enrichCallData]);
