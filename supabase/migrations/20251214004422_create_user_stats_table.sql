/*
  ...
  2. Security
    ...
    - Add policies for authenticated users to view all stats (for leaderboard)
...
CREATE POLICY "Authenticated users can update their own stats"
  ON user_stats
  FOR UPDATE
  TO authenticated
  USING (true)
...
CREATE POLICY "Authenticated users can delete their own stats"
  ON user_stats
  FOR DELETE
  TO authenticated
  USING (true);
